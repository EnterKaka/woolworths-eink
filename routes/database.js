        ...
        try {
            ...
            for (let model of models) {
                loadedData.filter((obj) => {
                    if (obj.name === model) result.push(obj);
                });
                modelname = model;
            }
            ...
            for (let element of settingid) {
                ...
                for (let model of models) {
                    const cursor = await datas
                        .find({ "measurement.name": model })
                        .toArray();
...
app.post("/importdb", async function (req, res) {
    ...
    async function run() {
        ...
        try {
            ...
            var type = req.body.filename.substr(
                req.body.filename.lastIndexOf(".") + 1
            );
            if (type === "json") {
                console.log("json*************************");
                let data = await fs.readFileSync(
                    __dirname + "/../upload/" + req.body.filename
                );
                ...
                await database
                    ...
                    .insertMany(docs_arr, function (err, result) {
                ...
                console.log("csv**********************");
                await csvtojson()
                    .fromFile(__dirname + "/../upload/" + req.body.filename)
                    .then(async (jsonobj) => {
                        await database
                            .collection(req.body.collectionname)
                            .insertMany(jsonobj, function (err, result) {
                                if (err) errorMsg = "Import failed";
